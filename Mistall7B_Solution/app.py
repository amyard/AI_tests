...
if 'vectorstore' not in st.session_state:
    # you can choose the persistence directory and load it from the disk
    st.session_state.vectorstore = Chroma(persist_directory='jj',
                                          embedding_function=OllamaEmbeddings(base_url='http://localhost:11434',
                                                                              model="mistral")
                                          )
#  initialize the llm and create a retriever
...
if uploaded_file is not None:
    ...
    if not os.path.isfile("files/"+uploaded_file.name):
        with st.status("Analyzing your document..."):
            ...
            # split the documents into chunks before embedding
